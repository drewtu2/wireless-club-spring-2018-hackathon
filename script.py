...
# required if you want to run this specific demo. If you have trouble 
# installing it, try any of the other demos that don't require it instead.
...
with open("people.csv", "r") as f:
    ...
    for row in reader:
        # Load a dan sample picture and learn how to recognize it.
        known_face_names.append(row[0])
        image = face_recognition.load_image_file(row[1])
        encoding = face_recognition.face_encodings(image)[0]

        known_face_encodings.append(encoding)
        bios[row[0]] = [row[2], False]
